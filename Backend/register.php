...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($email) || empty($password) || empty($confirm_password)) {
        echo json_encode(['success' => false, 'message' => 'Все поля, кроме ника, обязательны']);
        exit;
    }
    ...
    if ($nickname && (strlen($nickname) < 3 || strlen($nickname) > 50)) {
        echo json_encode(['success' => false, 'message' => 'Ник должен быть от 3 до 50 символов или пустым']);
        exit;
    }
    ...
    $stmt = $pdo->prepare("INSERT INTO users (username, nickname, email, password, ip_address, created_at) VALUES (?, ?, ?, ?, ?, NOW())");
    try {
        $stmt->execute([$username, $nickname ?: NULL, $email, $hashed_password, $ip_address]);
