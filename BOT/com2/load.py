    ...
    try:
        with get_db_connection() as conn, conn.cursor() as cursor:
            ...
            for booking_id, booking_time, booking_date in bookings:
                try:
                    booking_datetime = datetime.combine(booking_date, datetime.min.time()) + timedelta(
                        hours=booking_time)
...
def get_user_booking_count(telegram_id):
    cleanup_expired_bookings()
    try:
        with get_db_connection() as conn, conn.cursor() as cursor:
            if conn is None:
                return 0
            cursor.execute("SELECT COUNT(*) FROM bookings WHERE telegram_id = %s", (telegram_id,))
...
def book_computer(telegram_id, comp_id, hour, phone_number, telegram_username):
    now = datetime.now(pytz.timezone('Europe/Moscow'))
    booking_date = now.date() if now.hour < 19 else (now + timedelta(days=1)).date()
    ...
    try:
        with get_db_connection() as conn, conn.cursor() as cursor:
            ...
            cursor.execute(
                ...
                (booking_id, telegram_id, comp_id, hour, booking_date, phone_number, telegram_username, telegram_id)
            )
            conn.commit()
            logger.info(
                f"Booked computer: telegram_id={telegram_id}, computer_id={comp_id}, time={hour}, booking_id={booking_id}, username={telegram_username}")
            return booking_id
    except mysql.connector.Error as e:
...
def cancel_booking(telegram_id, booking_id):
    try:
        with get_db_connection() as conn, conn.cursor() as cursor:
            if conn is None:
                logger.error("No database connection available")
            ...
            cursor.execute(
                "SELECT booking_id, computer_id, booking_date, booking_time, phone_number, username FROM bookings "
                "WHERE booking_id = %s AND telegram_id = %s",
                (booking_id, telegram_id)
            )
            booking = cursor.fetchone()
            if not booking:
                logger.warning(f"Booking {booking_id} not found or not owned by telegram_id={telegram_id}")
                return False, "Бронирование не найдено или не принадлежит вам"
            cursor.execute("DELETE FROM bookings WHERE booking_id = %s", (booking_id,))
            conn.commit()

            # Формируем сообщение для уведомления администратора
            booking_id, computer_id, booking_date, booking_time, phone_number, username = booking
            message = (
                f"❌ *Бронирование отменено*\n\n"
                f"ID бронирования: {booking_id}\n"
                f"Компьютер: PC{computer_id}\n"
                f"Дата: {booking_date.strftime('%d.%m.%Y')}\n"
                f"Время: {booking_time}:00 - {booking_time + 1}:00\n"
                f"Пользователь: {username or '@unknown'}\n"
                f"Телефон: {phone_number}\n"
                f"Отменено пользователем: {username or '@unknown'}"
            )

            # Получаем конфигурацию Telegram бота
            cursor.execute("SELECT bot_token, admin_chat_id FROM telegram_bot_config LIMIT 1")
            config = cursor.fetchone()
            if config:
                bot_token, admin_chat_id = config
                send_telegram_message(bot_token, admin_chat_id, message)
            else:
                logger.error("Telegram bot configuration not found")

            logger.info(f"Cancelled booking: booking_id={booking_id}, telegram_id={telegram_id}")
    ...
    try:
        with get_db_connection() as conn, conn.cursor() as cursor:
            ...
            cursor.execute(
                ...
                "WHERE telegram_id = %s ORDER BY booking_date, booking_time",
                (telegram_id,)
            )
    ...
    try:
        with get_db_connection() as conn, conn.cursor() as cursor:
            if conn is None:
                return []
            cursor.execute(
                "SELECT booking_id, user_id, computer_id, booking_time, phone_number, booking_date, telegram_id FROM bookings")
            return cursor.fetchall()
    except mysql.connector.Error as e:
        ...
        try:
            ...
            for booking_id, user_id, computer_id, booking_time, phone_number, booking_date, telegram_id in bookings:
                ...
                if booking_datetime.date() == now.date() and booking_time == current_hour and 45 <= current_minute <= 50:
                    ...
                    logger.info(
                        f"Reminder sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not telegram_username:
        ...
        logger.info(
            f"No username error sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        await asyncio.sleep(0.2)
        return
    ...
    logger.info(
        f"Start message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
    await asyncio.sleep(0.2)
...
async def book(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if booking_count >= 2:
        ...
        logger.info(
            f"Booking limit message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        await asyncio.sleep(0.2)
        return
    if booking_count == 1:
        bookings = get_user_bookings(telegram_id)
        ...
        await update.message.reply_text(response, parse_mode='Markdown')
        logger.info(
            f"Current booking message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        await asyncio.sleep(0.2)
    ...
    try:
        with open(image_file, 'rb') as photo:
            ...
            logger.info(
                f"Computer selection photo sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        ...
        logger.info(
            f"Computer selection text sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
    await asyncio.sleep(0.2)
...
async def bookings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not bookings:
        ...
        logger.info(
            f"No bookings message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        await asyncio.sleep(0.2)
        return
    ...
    await update.message.reply_text(response, parse_mode='Markdown')
    logger.info(
        f"Bookings list sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
    await asyncio.sleep(0.2)
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        start_time = datetime.now()
        await update.message.reply_text(
            "ℹ️ Пожалуйста, укажите код бронирования для отмены. Например: `/cancel 1234`",
            parse_mode='Markdown'
        )
        logger.info(
            f"No booking_id provided message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        await asyncio.sleep(0.2)
        return
    booking_id = context.args[0].strip()
    success, message = cancel_booking(telegram_id, booking_id)
    start_time = datetime.now()
    await update.message.reply_text(
        message,
        parse_mode='Markdown'
    ...
    logger.info(
        f"Cancel result ({'success' if success else 'failure'}) sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
    await asyncio.sleep(0.2)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "book":
        ...
        if booked_hours:
            keyboard.append(
                [InlineKeyboardButton(f"~~{hour}:00~~", callback_data="unavailable") for hour in booked_hours])
        if not any(row for row in keyboard if any(btn.callback_data != "unavailable" for btn in row)):
            ...
            logger.info(
                f"No available hours message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
            await asyncio.sleep(0.2)
            return
        reply_markup = InlineKeyboardMarkup(keyboard)
        start_time = datetime.now()
        await query.message.reply_text(f"Выберите время для PC{comp_num}:", reply_markup=reply_markup,
                                       parse_mode='Markdown')
        logger.info(
            f"Time selection message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        await asyncio.sleep(0.2)
    elif query.data == "unavailable":
        start_time = datetime.now()
        await query.message.reply_text(f"Это время уже забронировано. 😔", parse_mode='Markdown')
        logger.info(
            f"Unavailable time message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        await asyncio.sleep(0.2)
        ...
        if is_computer_booked(comp_num, hour):
            ...
            logger.info(
                f"Booked time message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
            await asyncio.sleep(0.2)
            return
        ...
        logger.info(
            f"Phone number request sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        await asyncio.sleep(0.2)
        ...
        if not all([comp_num, hour, phone_number]):
            start_time = datetime.now()
            await query.message.reply_text("Ошибка: данные бронирования устарели. Начните заново с /start.",
                                           parse_mode='Markdown')
            logger.info(
                f"Expired booking data message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        ...
        if is_computer_booked(comp_num, hour):
            ...
            await asyncio.sleep(0.2)
            return
        booking_id = book_computer(telegram_id, comp_num, hour, phone_number, telegram_username)
        if not booking_id:
            start_time = datetime.now()
            await query.message.reply_text("Не удалось создать бронирование. Попробуйте снова.", parse_mode='Markdown')
            logger.info(
                f"Booking failure message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
            context.user_data.clear()
            await asyncio.sleep(0.2)
            return
        now = datetime.now(pytz.timezone('Europe/Moscow'))
        ...
        logger.info(
            f"Booking confirmation sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        context.user_data.clear()
        await asyncio.sleep(0.2)
        ...
        logger.info(
            f"Booking cancellation message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        context.user_data.clear()
        await asyncio.sleep(0.2)
...
async def handle_phone_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'comp_num' not in context.user_data or 'hour' not in context.user_data:
        ...
        logger.info(
            f"Invalid booking data message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        await asyncio.sleep(0.2)
        return
    ...
    if timestamp and (datetime.now() - timestamp).total_seconds() > 300:
        ...
        logger.info(
            f"Timeout message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        context.user_data.clear()
        await asyncio.sleep(0.2)
        return
    ...
    if not phone_number.startswith("+") or len(phone_number) < 10:
        ...
        logger.info(
            f"Invalid phone number message sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
        await asyncio.sleep(0.2)
        return
    if get_user_booking_count(telegram_id) >= 2:
        bookings = get_user_bookings(telegram_id)
    ...
    logger.info(
        f"Booking confirmation prompt sent to telegram_id={telegram_id} in {(datetime.now() - start_time).total_seconds()} seconds")
    await asyncio.sleep(0.2)
